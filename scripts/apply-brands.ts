...
async function main() {
  ...
  if (!DRY && redirectLines.length) {
    const blockHeader = `# Brand re-slug 301s — ${city}/${category} (generated by apply-brands.ts)`;
    // Remove any existing block for this city/category before appending the fresh one.
    let existing = fs.existsSync(REDIRECTS) ? fs.readFileSync(REDIRECTS, "utf8") : "";
    // Strip the leading blank line + header + all contiguous non-empty redirect lines that follow it.
    const escaped = blockHeader.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    existing = existing.replace(new RegExp(`\\n${escaped}\\n(?:[^\\n]+\\n)*`, "g"), "");
    const block = `\n${blockHeader}\n` + redirectLines.join("\n") + "\n";
    fs.writeFileSync(REDIRECTS, existing + block, "utf8");
    console.log(`Wrote ${redirectLines.length} redirect lines → ${REDIRECTS}`);
  }
}
