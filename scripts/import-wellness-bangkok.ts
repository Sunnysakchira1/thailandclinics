/**
 * Import curated Bangkok wellness clinics → Turso (wellness-clinics / bangkok).
 * Reads raw Outscraper merge (data/wellness_bangkok_raw.json).
 *
...
const SLUG_OVERRIDES: Record<string, string> = {
  "ChIJt_oDkimd4jARp3Ed9cPPc1Y": "th-iv-mobile-drip",                    // Hangover/Jetlag… Mobile IV Drip by TH.IV
  "ChIJ7ctNzeaf4jARELwSN-pY95A": "primecare-clinic-sukhumvit",          // PrimeCare Sukhumvit
  "ChIJjYOh5Iqf4jAR7JYSzDAvg8Q": "primecare-clinic-sathorn",            // PrimeCare Sathorn
  "ChIJAy_0H2Cf4jARv0yWW-XSipg": "pulse-clinic-phromphong",             // PULSE Phromphong (was … | …)
  "ChIJSRHFe1ef4jARlkSWXaGLj6c": "stc-anti-aging-wellness-sukhumvit-39",
  "ChIJf7zYWnWd4jARd0E6bT4GmsE": "stc-anti-aging-wellness-phaholyothin-32",
};
...
// Medical + holistic wellness ONLY. Strip: spas/massage/yoga, skin/derm/cosmetic
// (belong in cosmetic), physio/dental/urology/hair, product shops.
...
const isJunk = (cat: string | null): boolean => {
  if (!cat) return false;
  return NOISE.test(cat);
};

async function main() {
  const raw: Record<string, unknown>[] = JSON.parse(fs.readFileSync("data/wellness_bangkok_raw.json", "utf8"));
  console.log(`Loaded ${raw.length} raw places`);

  // Some spa/derm/aesthetic clinics have a clean category type but give themselves
  // away in the name — strip those too (keeps the category medical-wellness, not cosmetic).
  ...
  if (!city || !cat) { console.error("Missing bangkok city or wellness-clinics category"); process.exit(1); }
