/**
 * Fetch last 30 English reviews per clinic for COSMETIC Bangkok clinics only (category_id = 3)
 * → clinic_reviews. Same Outscraper REST + batching as fetch-reviews.ts, but
 * scoped so it does NOT re-pull (and re-bill) the physiotherapy clinics.
 *
 * Usage: npx tsx --env-file=.env.local scripts/fetch-reviews-cosmetic-bangkok.ts
 */
...
async function main() {
  ...
  const rows = await db.select({ id: clinics.id, placeId: clinics.googlePlaceId })
    .from(clinics).where(and(eq(clinics.categoryId, 3), eq(clinics.cityId, 1)));
  const list = rows.filter((c) => c.placeId) as { id: number; placeId: string }[];
  console.log(`Cosmetic Bangkok clinics to fetch reviews for: ${list.length}`);
  ...
  console.log(`\nDone. Inserted ${inserted} reviews for Cosmetic Bangkok.`);
}
