/**
 * Import curated Bangkok IVF / fertility clinics → Turso (fertility-clinics / bangkok).
 * Reads raw Outscraper merge (data/fertility_bangkok_raw.json).
 *
...
// Clean slugs for names whose auto-slug is broken (Thai-name-with-parenthetical, etc.)
const SLUG_OVERRIDES: Record<string, string> = {
  "ChIJhW9fE76f4jARejCw9aAX5DA": "deep-harmonicare-ivf-center", // was "thailand"
  "ChIJLwPsTkmd4jARx9-em0BVG3g": "smile-ivf-clinic",            // was "bangkok"
  "ChIJdU0iYCSf4jAR19VjfT3jyW8": "nawabutr-ivf-center",         // ศูนย์การแพทย์นวบุตร
  "ChIJM1MMT56b4jARbtcHeH1369k": "gina-fertility-studio",       // จีน่าเฟอร์ทิลิตี้
  "ChIJrQ23CqOZ4jARDhL1rodQEI4": "jintabutr-fertility-center",  // จินตบุตร
  "ChIJj7IaqLqj4jAR4fDFygbePY0": "beyond-ivf-rama-2",           // was "rama-ii"
  "ChIJEey8K9xjHTERjHUgLaEeFD8": "siam-fertility-kaset",        // Siam Fertility @Kaset
  "ChIJzYnLGgBhHTERRq8L4H9ra7w": "genesis-fertility-center",    // disambiguate from genesis-fertility-center-plc
};

// Force-include despite the bar (landmark institutions). Still must be OPERATIONAL + Bangkok.
const ALWAYS_INCLUDE = new Set<string>([
  "ChIJwWrv68Se4jAR2TmoiFIGbxM", // Jetanin Hospital — 4.1★, IVF pioneer since 1996
]);
...
async function main() {
  const raw: Record<string, unknown>[] = JSON.parse(fs.readFileSync("data/fertility_bangkok_raw.json", "utf8"));
  ...
  const [cat] = await db.select().from(categories).where(eq(categories.slug, "fertility-clinics"));
  if (!city || !cat) { console.error("Missing bangkok city or fertility-clinics category"); process.exit(1); }
